# Sang_hoon Lee's Portfolio related to Data Science.

#Career
1. 국립금오공과대학교 응용수학과 (2020 ~ 2021) 
2. 국립금오공과대학교 수리빅데이터학과(2022 ~ 2026)
3. 중앙대학교 통계데이터사이언스학과 (2026 ~ )
...
#Personal Projects
1. 거시경제 변동에 따른 오프라인 골목상권 영향도 분석 (2026)
2. 
   -기준금리 변동이 자영업자 매출에 미치는 직접적인 타격 여부를 검증하고, 오프라인 매출 변동을 유발하는 진짜 거시경제 리스크 동인을 규명함.
   
   -종로구 6개 업종별 월간 신한카드 결제 데이터와 한국은행 거시경제 지표 3종(금리, 물가, 심리)을 활용하였으며, 다중공선성 통제와 체급 평준화를 위해 Log1p 및 Z-Score 전처리를 수행함.
   
   -소표본 데이터의 노이즈와 과적합 문제를 해결하기 위해, 6개 업종을 공통의 경제 흐름 아래 종속된 구조로 학습시키는 '부모-자식 구조의 베이지안 계층 모델(BHM)'을 구축하여 MCMC 시뮬레이션으로 검증함.
